...
class PositiveDataset(Dataset):
    ...
    def __init__(self, target_concepts, name_table, name_bridge, max_elements=10, label = 1):
        ...
        self.filtered_bridge = name_bridge.sample(frac=1, random_state=42).groupby('concept_id').head(self.max_elements)
    ...
    def __str__(self):
        ...
        return f"PositiveDataset(length={len(self)}, label={self.label})"
...
class NegativeDataset(Dataset):
    ...
    def __init__(self, target_concepts, name_table, blacklist_bridge, max_elements=10, seed=42):
        self.target_concepts = target_concepts
        self.name_table = name_table
        self.blacklist_bridge = blacklist_bridge
        self.max_elements = max_elements
        ...
        bridge = pd.DataFrame({
            'concept_id': np.repeat(self.all_concept_ids, max_elements),
            'name_id': self.rng.choice(self.all_name_ids, size=len(self.all_concept_ids) * max_elements, replace=True)
        }).drop_duplicates()
        bridge = bridge.set_index(['concept_id', 'name_id'])
        blacklist_bridge = blacklist_bridge.set_index(['concept_id', 'name_id'])
        ...
        missing_counts = counts[counts < max_elements]
        ...
        for concept_id, count in zip(missing_counts.index, missing_counts.values):
            ...
            num_sample = max_elements - len(selected)
            if num_sample > 0:
                new_samples = self._get_random_sample(concept_id, selected, num_sample)
        ...
        bridge = bridge.groupby('concept_id').head(max_elements)
        self.bridge = bridge
    
        
    def _get_random_sample(self, concept_id, selected, num_sample):
        ...
        if num_sample > 0:
            return self.rng.choice(candidates, size=num_sample, replace=False)
        return []
    ...
    def __getitem__(self, idx):
        ...
        sentence1 = self.concept_id_2_name[concept_id1]
        sentence2 = self.name_id_2_name[concept_id2]
        return {"sentence1": sentence1, "sentence2": sentence2, "label": 1}
    ...
    def _get_single_item(self, idx):
        ...
        for i, partial_len in enumerate(self.partial_lengths):
            if idx < partial_len:
                ...
                local_idx = idx - previous_len
                return dataset.iloc[local_idx] if isinstance(dataset, pd.DataFrame) else dataset[local_idx]
        raise IndexError("Index out of range")
    
    def __str__(self):
        return f"CombinedDataset(length={len(self)}, datasets={self.names})"
